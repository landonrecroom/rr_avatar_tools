...
class RR_OT_SetupEnsureLinkedLibraries(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        return bpy.ops.rr.cleanup_remove_deprecated_libraries.poll() or bpy.ops.rr.cleanup_fix_broken_libraries.poll()
...
class RR_OT_SetupImportFullBodyMeshes(RecRoomAvatarOperator):
    ...
    def execute(self, context):
        ...
        with bpy.data.libraries.load(resources.fb_library, link=True, relative=True) as (data_from, data_to):
            if not has_collection:
                data_to.collections = [c for c in data_from.collections if c == 'FB_Resources']
        ...
        if not has_collection:
            bpy.context.view_layer.active_layer_collection = self.get_view_layer('Full_Body')
...
class RR_OT_SetupImportModernBeanBodyMeshes(RecRoomAvatarOperator):
    ...
    def execute(self, context):
        ...
        with bpy.data.libraries.load(resources.mb_library, link=True, relative=True) as (data_from, data_to):
           data_to.collections = [c for c in data_from.collections if c == 'MB_Resources']

        bpy.context.view_layer.active_layer_collection = self.get_view_layer('Modern_Bean_Body')
...
class RR_OT_SetupTestOutfits(bpy.types.Operator):
    ...
    def execute_(self, context):
        ...
        with bpy.data.libraries.load(resources.fb_library, link=True, relative=True) as (data_from, data_to):
            data_to.collections = [c for c in data_from.collections if c == 'FB_TestOutfits']
        ...
        with bpy.data.libraries.load(resources.mb_library, link=True, relative=True) as (data_from, data_to):
            data_to.collections = [c for c in data_from.collections if c == 'MB_TestOutfits']
